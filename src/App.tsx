...
export default function App() {
  ...
  const startWorkout = async () => {
    savingRef.current = false; repsRef.current = 0; setReps(0); setFeedback('Colócate de lado'); setCameraError(''); setView('workout')
  ...
  if (view === 'workout') return <main className="workout"><button className="back" onClick={() => void finish()}>×</button><div className="camera"><video ref={videoRef} className={facingMode === 'user' ? 'mirrored' : ''} muted playsInline/><button className="switch-camera" onClick={() => void requestCamera(facingMode === 'user' ? 'environment' : 'user')} aria-label="Alternar cámara">↺ <span>{facingMode === 'user' ? 'Frontal' : 'Trasera'}</span></button><div className="counter overlay-counter">{reps}<span> / {goal}</span></div><div className="camera-label">POSE LANDMARKER · PROCESAMIENTO LOCAL</div></div><div className="feedback" aria-live="polite">{feedback}</div>{cameraError && <p className="notice">{cameraError}</p>}<div className="progress"><i style={{ width: `${Math.min(100, reps / goal * 100)}%` }}/></div><div className="controls"><button className="round" onClick={removeManualRep}>−</button><button className="rep" onClick={addRep}>+1</button><button className="round" onClick={() => void finish()}>✓</button></div><p className="hint">La cámara analiza tu postura localmente. Usa +1 solo como respaldo.</p></main>
