...
import type { PushUpFeedback } from './pushUpCounter'
...
export default function App() {
  ...
  useEffect(() => { refresh() }, [])
  const stopCamera = useCallback(() => { stream?.getTracks().forEach(track => track.stop()); setStream(null); detector.stop() }, [stream, detector])
  ...
  useEffect(() => { detector.onRep(addRep); detector.onFeedback(setFeedback) }, [detector, addRep])
  ...
  const startWorkout = async () => {
    stopCamera(); savingRef.current = false; repsRef.current = 0; setReps(0); setFeedback('Colócate de lado'); setCameraError(''); setView('workout')
    try {
      const cameraStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment', width: { ideal: 1280 }, height: { ideal: 720 } }, audio: false })
      setStream(cameraStream)
    } catch { setCameraError('No se pudo acceder a la cámara. Puedes continuar con el contador manual.') }
  }
  const removeManualRep = () => { const next = Math.max(0, repsRef.current - 1); repsRef.current = next; setReps(next) }
  if (view === 'workout') return <main className="workout"><button className="back" onClick={() => void finish()}>×</button><div className="camera"><video ref={videoRef} muted playsInline/><div className="counter overlay-counter">{reps}<span> / {goal}</span></div><div className="camera-label">POSE LANDMARKER · PROCESAMIENTO LOCAL</div></div><div className="feedback" aria-live="polite">{feedback}</div>{cameraError && <p className="notice">{cameraError}</p>}<div className="progress"><i style={{ width: `${Math.min(100, reps / goal * 100)}%` }}/></div><div className="controls"><button className="round" onClick={removeManualRep}>−</button><button className="rep" onClick={addRep}>+1</button><button className="round" onClick={() => void finish()}>✓</button></div><p className="hint">La cámara analiza tu postura localmente. Usa +1 solo como respaldo.</p></main>
